...
async fn test_resolve_download_different_urls_with_same_hash() -> anyhow::Result<()> {
    ...
    let file_1_download = LazyValue::Download(DownloadValue {
        hash: hello_hash.clone(),
    ...
    let file_2_download = LazyValue::Download(DownloadValue {
        hash: hello_hash.clone(),
...
async fn test_resolve_download_url_changed_hash() -> anyhow::Result<()> {
    ...
    let file_download_1 = LazyValue::Download(DownloadValue {
        hash: hello_hash.clone(),
        url: format!("{server_url}/file.txt").parse().unwrap(),
    });
    ...
    let file_download_2 = LazyValue::Download(DownloadValue {
        hash: hi_hash.clone(),
        url: format!("{server_url}/file.txt").parse().unwrap(),
    });
...
async fn test_resolve_download_does_not_cache_using_only_hash() -> anyhow::Result<()> {
    ...
    // wrong thing)
    let invalid_hi_download = LazyValue::Download(DownloadValue {
        hash: hello_hash.clone(),
        url: format!("{server_url}/hi.txt").parse().unwrap(),
    });

    let hi_download = LazyValue::Download(DownloadValue {
        hash: hi_hash.clone(),
        url: format!("{server_url}/hi.txt").parse().unwrap(),
    });
