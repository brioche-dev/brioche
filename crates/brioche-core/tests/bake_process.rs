...
fn brioche_test<Fut, T>(f: impl FnOnce(brioche_core::Brioche) -> Fut) -> T
where
    Fut: Future<Output = T>,
{
    ...
    runtime.block_on(async move {
        let (brioche, _context) = brioche_test_support::brioche_test_with(|mut brioche| {
...
#[test]
fn test_bake_process_simple() -> anyhow::Result<()> {
...
#[test]
fn test_bake_process_fail_on_no_output() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
...
#[test]
fn test_bake_process_scaffold_output() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let hello_blob = brioche_test_support::blob(&brioche, "hello").await;
...
#[test]
fn test_bake_process_scaffold_and_modify_output() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let hello_blob = brioche_test_support::blob(&brioche, "hello").await;
...
#[test]
fn test_bake_process_fail_on_non_zero_exit() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let process = Recipe::Process(ProcessRecipe {
            command: tpl("/usr/bin/env"),
            args: vec![
...
#[test]
fn test_bake_process_command_no_path() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let process = Recipe::Process(ProcessRecipe {
            command: tpl("env"),
            args: vec![tpl("sh"), tpl("-c"), tpl("echo -n hello > $BRIOCHE_OUTPUT")],
...
// than `execvpe`
#[test]
fn test_bake_process_command_path() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let process = Recipe::Process(ProcessRecipe {
            command: tpl("env"),
            args: vec![tpl("sh"), tpl("-c"), tpl("echo -n hello > $BRIOCHE_OUTPUT")],
...
#[test]
fn test_bake_process_with_utils() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let process = Recipe::Process(ProcessRecipe {
            command: tpl("/usr/bin/env"),
...
#[test]
fn test_bake_process_with_readonly_contents() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let process = Recipe::Process(ProcessRecipe {
            command: tpl("/usr/bin/env"),
            args: vec![
...
#[test]
fn test_bake_process_cached() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
...
#[test]
fn test_bake_process_cached_equivalent_inputs() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
...
#[test]
fn test_bake_process_cached_equivalent_inputs_parallel() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let empty_dir_1 = brioche_test_support::lazy_dir_empty();
        let empty_dir_2 = Recipe::Process(ProcessRecipe {
            command: tpl("/usr/bin/env"),
...
#[test]
fn test_bake_process_cache_busted() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
...
#[test]
fn test_bake_process_custom_env_vars() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
...
#[test]
fn test_bake_process_no_default_env_vars() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let process = ProcessRecipe {
            command: tpl("/usr/bin/env"),
            args: vec![
...
#[test]
fn test_bake_process_no_default_path() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let process = ProcessRecipe {
            command: tpl("sh"),
            args: vec![
...
#[test]
fn test_bake_process_command_ambiguous_error() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
...
#[test]
fn test_bake_process_command_uses_path() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let process = ProcessRecipe {
            command: tpl("sh"),
            args: vec![
...
#[test]
fn test_bake_process_command_uses_dependencies() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let process = ProcessRecipe {
            command: tpl("sh"),
            args: vec![
...
#[test]
fn test_bake_process_starts_with_work_dir_contents() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
...
#[test]
fn test_bake_process_has_resource_dir() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let process = ProcessRecipe {
            command: tpl("/usr/bin/env"),
            args: vec![
...
#[test]
fn test_bake_process_contains_all_resources() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
...
#[test]
fn test_bake_process_output_with_resources() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
...
#[test]
fn test_bake_process_output_with_shared_resources() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        // Create a dummy file with pack metadata. This attaches resources
        // to the file when it gets put into the output of a process.
        ...
        let dummy_packed_1_blob =
            brioche_test_support::blob(&brioche, &dummy_packed_1_contents).await;
        ...
        let dummy_packed_2_blob =
            brioche_test_support::blob(&brioche, &dummy_packed_2_contents).await;
...
#[test]
fn test_bake_process_output_with_nested_symlink_resource_error() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        // Create a dummy file with pack metadata. This attaches resources
        // to the file when it gets put into the output of a process.
        let dummy_packed_contents = "dummy_packed".to_string();
...
#[test]
fn test_bake_process_output_with_symlink_traversal_resource_error() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        // Create a dummy file with pack metadata. This attaches resources
        // to the file when it gets put into the output of a process.
        let dummy_packed_contents = "dummy_packed".to_string();
...
#[test]
fn test_bake_process_unsafe_validation() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
...
#[test]
fn test_bake_process_networking_disabled() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
...
#[test]
fn test_bake_process_networking_enabled_dns() -> anyhow::Result<()> {
    brioche_test(|brioche| async move {
        let process = Recipe::Process(ProcessRecipe {
            command: tpl("/usr/bin/env"),
            args: vec![
                            ...
                            brioche_test_support::dir(
                                ...
                                [
                                    ...
                                    (
                                        "DEP1_FILE",
                                        brioche_test_support::file(
                                            brioche_test_support::blob(&brioche, b"dep1_file")
                                                .await,
                                            false,
                                        ),
                                    ),
                            ...
                            brioche_test_support::dir(
                                ...
                                [
                                    ...
                                    (
                                        "DEP2_FILE",
                                        brioche_test_support::file(
                                            brioche_test_support::blob(&brioche, b"dep2_file")
                                                .await,
                                            false,
                                        ),
                                    ),
