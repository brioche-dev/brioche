...
impl JsRuntime {
    ...
    pub async fn get_export(
        ...
        export: &str,
    ) -> Result<JsValueHandle, EvaluateError> {
        let (result_tx, result_rx) = tokio::sync::oneshot::channel();
        self.tx
            .send(JsRuntimeMessage::GetExport {
                project_ref,
    ...
    pub async fn with_context<T>(
        &self,
        f: impl AsyncFnOnce(&mut JsContext) -> T + Send + 'static,
        ...
        T: Send + 'static,
    {
        let (result_tx, result_rx) = tokio::sync::oneshot::channel();
        self.tx
            .send(JsRuntimeMessage::WithContext {
                f: Box::new(move |ctx| {
                    Box::pin(async move {
...
enum JsRuntimeMessage {
    ...
    GetExport {
        project_ref: ProjectRef,
        export: String,
        result_tx: tokio::sync::oneshot::Sender<Result<JsValueHandle, EvaluateError>>,
    },
    WithContext {
        f: Box<WithContextFn>,
    },
...
type WithContextFn = dyn for<'a> FnOnce(&'a mut JsContext<'_, '_, '_, '_, '_>) -> future::LocalBoxFuture<'a, ()>
    + Send;
...
impl JsRuntimeBridge {
    ...
    async fn handle_message(&mut self, message: JsRuntimeMessage) {
        match message {
            ...
            JsRuntimeMessage::GetExport {
                project_ref,
                export,
                result_tx,
            } => {
                let result = self.get_export(project_ref, export).await;
                let _ = result_tx.send(result);
            }
            JsRuntimeMessage::WithContext { f } => {
                ...
                f(&mut JsContext {
                    ...
                    next_value_id: &mut self.next_value_id,
                })
                .await;
            }
        }
    ...
    async fn get_export(
        ...
        let value = {
            ...
            let export_key = deno_core::v8::String::new(js_scope, &export)
                .ok_or_else(|| EvaluateError::InvalidJsString(export.clone()))?;
    ...
    fn add_value(&mut self, value: deno_core::v8::Global<deno_core::v8::Value>) -> JsValueHandle {
        let value_id = self.next_value_id;
        self.next_value_id = self.next_value_id.checked_add(1).unwrap();
        let handle = Arc::new(value_id);

        self.values
            .insert(value_id, (Arc::downgrade(&handle), value));
        JsValueHandle(handle)
    }
}
